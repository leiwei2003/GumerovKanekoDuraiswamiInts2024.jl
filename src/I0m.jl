function I0m(P, h1, h2, h3, h4)
    ...
    if h1^2 + h2^2 < zero2 * hh # if h1 = h2 = 0
        I = 0.0 # based on the limit of case 6 as h1 approaches 0
    elseif h2^2 < zero2 * hh # if h2 = 0 -> Case 6
        if P < zerotol
            Phi2 = h1 / (hh + h4 * R)
        else
        ...
        I = (
            Phi1 + (h1^2 - h4^2) / (2 * h1 * h4) * Phi2
            - Phi3 + 0.5 / (R + h4)
            ) / h1^2 # Case 6
    elseif h1^2 < zero2 * hh # if h1 = 0 -> Case 7
        h = sqrt(hh)
        R2 = sqrt(P^2 + h2^2)
        Phi2 = atan(h2 * P / (hh + h4 * R)) / P
        Phi4 = h4^2 / (h2 * P^2) * ((R2 / h2 * log((R2 + R) / h4) - log((h2 + h) / h4)))
        I = (
            - Phi1 + h4 / h2 * Phi2 + (h2 / h4)^2 * Phi4
            - (R2^2 / (R + h4) - h2^2 / (h + h4)) / P^2
            ) / h2^2 # Case 7
    end
